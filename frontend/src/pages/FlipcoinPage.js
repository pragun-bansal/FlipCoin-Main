      ...
      <div className='flex flex-row px-10 gap-x-6 flipcoinpage'>
        ...
        <div className="bg-white h-[80vh] mt-24 border-gray rounded-2xl shadow-md  overflow-y-scroll w-2/3 beautifulscroller">
            ...
            <div className="flex flex-row justify-between items-center w-full cursor-pointer border-gray-200 border-b-2">
              <div className={`flex flex-row justify-between items-center w-1/2 p-4 rounded-t-2xl ${(value===0)?'bg-gray-200':''}`} onClick={()=>setValue(0)}>
                <p className="text-gray-800 text-lg text-center w-full  border-gray-200">Sent Transactions</p>
              </div>
              <div className={`flex flex-row justify-between items-center w-1/2 p-4 rounded-t-2xl ${(value===1)?'bg-gray-200':''}`} onClick={()=>setValue(1)}>
                <p className="text-gray-800 text-lg text-center w-full border-gray-200">Recived Transactions</p>
              </div>
            </div>
            ...
            {value === 0 ? 
              ...
              <table class="min-w-full text-left text-sm font-light">
                <thead class="border-b font-medium dark:border-neutral-500">
                  <tr>
                    ...
                    <th scope="col" class="px-6 py-4">Sent to </th>
              ...
              <tbody>
                {sentTransactions.map((tx, index) => (
                  <tr class="border-b transition duration-300 ease-in-out hover:bg-neutral-200" key={index}>
                    <td class="whitespace-nowrap px-6 py-4 font-medium">{index+1}</td>
                    <td class="whitespace-nowrap px-6 py-4">{tx.to}</td>
                    <td class="whitespace-nowrap px-6 py-4 text-red-400 font-bold">{tx.amount} FLC</td>
              ...
              :   
                ...
                <thead class="border-b font-medium dark:border-neutral-500">
                  <tr>
                    <th scope="col" class="px-6 py-4">S.No</th>
                    <th scope="col" class="px-6 py-4">Recived from </th>
                    <th scope="col" class="px-6 py-4">Amount</th>
                    <th scope="col" class="px-6 py-4">Date</th>
                  </tr>
                </thead>
              ...
              <tbody>
                {receivedTransactions.map((tx, index) => (
                  <tr class="border-b transition duration-300 ease-in-out hover:bg-neutral-200" key={index}>
                    ...
                    <td class="whitespace-nowrap px-6 py-4 text-green-400 font-bold">{tx.amount} FLC</td>
                    <td class="whitespace-nowrap px-6 py-4">{tx.date}</td>
                  </tr>
                ))}
              </tbody>
        ...
        <div class="wallet-header-summary">
          <div class="summary-text">
            Your Flipcoins Balance
          </div>
          <div class="summary-balance text-lg">
            {cryptoBalance} FLCs
          </div>
