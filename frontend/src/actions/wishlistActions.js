...
export const addToWishlist = (item) => async (dispatch, getState) => {
  ...
  if (isAuthenticate) {
    try {
      await axios.post(`${BACKEND_URL}/wishlist/add-item`, {
        userId: user._id,
        productId: item._id,
      });
    } catch (error) {}
...
export const removeFromWishlist = (id) => async (dispatch, getState) => {
  ...
  if (isAuthenticate) {
    try {
      await axios.delete(`${BACKEND_URL}/wishlist/remove-item`, {
        data: {
          userId: user._id,
          productId: id,
        },
...
export const getWishlistItems = () => async (dispatch, getState) => {
  ...
  if (isAuthenticate) {
    try {
      const { data } = await axios.get(`${BACKEND_URL}/wishlist/get-items/${user._id}`);
      const wishlistItems = [];
