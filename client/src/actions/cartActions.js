...
export const addToCart = (item) => async (dispatch, getState) => {
  ...
  if (!existItem) {
    if (isAuthenticate) {
      try {
        await axios.post(`${BACKEND_URL}/cart/add-item`, {
          userId: user._id,
          productId: item._id,
        });
      } catch (error) {}
...
export const removeFromCart = (id) => async (dispatch, getState) => {
  ...
  if (isAuthenticate) {
    try {
      await axios.delete(`${BACKEND_URL}/cart/remove-item`, {
        data: {
          userId: user._id,
          productId: id,
        },
...
export const clearCart = () => async (dispatch, getState) => {
  ...
  if (isAuthenticate) {
    try {
      await axios.delete(`${BACKEND_URL}/cart/clear-cart`, {
        data: {
          userId: user._id,
        },
      });
...
export const getCartItems = () => async (dispatch, getState) => {
  ...
  if (isAuthenticate) {
    try {
      const { data } = await axios.get(`${BACKEND_URL}/cart/get-items/${user._id}`);
      if (data.length > 0) {
        data?.map((value) => {
          var isExist = false;
